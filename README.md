...
**Google Apps Script + Google Sheets**. **Tanpa login** — identitas menempel
pada perangkat (HP) yang didaftarkan & disetujui admin.

Fitur:

- 📱 **Berbasis perangkat**: tiap HP didaftarkan (nama) lalu **disetujui admin**.
...
- 📋 **Rekap**: tiap perangkat melihat datanya sendiri; **admin (password) melihat semua**. Ekspor CSV.
- 🛡️ **Panel admin** (password): setujui/blokir/hapus perangkat, atur lokasi, jam kerja, buffer, mode uji coba, ganti password.

```
Browser (perangkat) ─→ GitHub Pages (statis)
       │  kirim Device ID + data
       ▼
 Google Apps Script  ← validasi lokasi/izin, filter rekap per-perangkat
       │
   ┌───┴────┐
   ▼        ▼
 Sheets   Drive (foto)
...
> **Catatan model perangkat.** "Punya sendiri" = data dari HP ini. Identitas
> tersimpan di browser HP (localStorage). Jika cache/aplikasi browser dibersihkan
> atau ganti HP, perangkat perlu didaftarkan & disetujui ulang. Validasi lokasi &
> izin dilakukan di server. Memadai untuk kebutuhan instansi; bukan anti-bobol mutlak.
...
3. Jalankan fungsi **`setup`** sekali (izinkan akses). Membuat tab **Absensi**,
   **Jurnal**, **Perangkat** dan **password admin awal** (`admin123`, lihat **View → Logs**).
...
1. Buka **`admin.html`** → login password awal **`admin123`**.
...
5. **Ganti Password Admin** (disarankan). **Simpan**.

---

## Alur Penggunaan

1. **Pegawai**: buka situs → form **Daftarkan Perangkat** (nama, NIP) → *pending*.
...
4. **Rekap**: `rekap.html` → otomatis tampil data perangkat ini; admin isi
   **Password Admin** untuk lihat semua → ekspor CSV.
...
penuh diperlukan PWA + push / aplikasi native. Izinkan **Notifikasi** saat diminta.

## Catatan Lain

- **HTTPS wajib** (GitHub Pages otomatis) untuk GPS & kamera.
- 1 HP = 1 perangkat. Foto jurnal tersimpan di folder **Foto Jurnal PJLP** pada Drive admin.
...
js/config.js                           Konfigurasi + ID perangkat + API
